...
void CTraderSpi::OnRspQryTradingAccount(CThostFtdcTradingAccountField *pTradingAccount, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (bIsLast && !IsErrorRspInfo(pRspInfo))
	{
		cout << "上次质押金额 " << pTradingAccount->PreMortgage << endl;
		cout << "上次信用额度 " << pTradingAccount->PreCredit << endl;
		cout << "上次存款额 " << pTradingAccount->PreDeposit << endl;
		cout << "上次结算准备金 " << pTradingAccount->PreBalance << endl;
		cout << "上次占用的保证金 " << pTradingAccount->PreMargin << endl;
		cout << "利息基数 " << pTradingAccount->InterestBase << endl;
		cout << "利息收入 " << pTradingAccount->Interest << endl;
...
//	TThostFtdcUserIDType	UserID;
	...
	///价格
	req.LimitPrice = 0;//市价单直接写0即可 THOST_FTDC_OPT_AskPrice1;//卖一价
	///数量: 1
